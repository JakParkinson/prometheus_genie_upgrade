## *Preface* 
The following preface is written by Jack Parkinson

This project is the adapted Prometheus framework with additions made during my MSc thesis (thesis: https://github.com/JakParkinson/Master-s-Thesis/blob/main/MSc_thesis.pdf )

### First and formost, how to run an Example:

## Simple IceCube Upgrade Example

A small working example of the GENIE, Prometheus, and ceCube Upgrade simulation chain is provided in [`simple_upgrade_example/`](simple_upgrade_example/).

The example uses GENIE gRooTracker events as input, injects and rotates the final-state particles in the IceCube Upgrade volume, and propagates the events through Prometheus using the IceCube Upgrade geometry and optical modules.

The folder includes small example GENIE input files as well as example Prometheus output files, allowing the simulation and output format to be explored without first generating a large GENIE sample.

### Running the example

From the `simple_upgrade_example` directory:

```bash
cd simple_upgrade_example
python run_external_genie.py
```

By default, this runs the included **2-event GENIE sample** as a quick test of the full simulation chain.

The same example can be run explicitly with:

```bash
python run_external_genie.py \
    --nseed 5 \
    --rootfile input/genie_numu_2events.gtrac.root
```

A **100-event GENIE sample** is also included for a slightly larger example:

```bash
python run_external_genie.py \
    --nseed 5 \
    --rootfile input/genie_numu_100events.gtrac.root
```

By default, the intermediate primary and particle tables are removed after a successful simulation. To keep them, use:

```bash
python run_external_genie.py \
    --nseed 5 \
    --rootfile input/genie_numu_100events.gtrac.root \
    --keep-intermediate
```

Simulation outputs are written to `simple_upgrade_example/output/`.

For the included 100-event example, the available output files are:

```text
primary_events_100.parquet
particle_events_100.parquet
simulated_events_100.parquet
```

These correspond to the GENIE primary-event information, the injected final-state particle information, and the final Prometheus detector simulation output, respectively.

The included output files can also be inspected directly without rerunning the simulation. There is a dedicated ipynb file in this folder showing how to do that.



## New changes to Prometheus: 
### GENIE injection extensibility
There are 2 different ways to simulate with GENIE. The recommended way is to provide external GENIE ROOT files generated by the user. This allows for the user to chose their specific parameters and cross-sections. The integration code for the gtrac file -> Prometheus method is located in genie_examples/genie_external_root_upgrade.py. 
The other option is to inject with a 'preset' GENIE gevgen sim at genie_examples/example_genie_full_injection2.py. This method injects with gevegen and sets the specific arguements found in genie_examples/gevgen_injection.py. If possible use GENIE version 3.04.02.


For more information see Chapter 4 in my MSc thesis.

### IceCube Upgrade PPC Tables
For new IceCube upgrade tables see resource/PPC_tables/upgrade_tables. These have the effective area table for mDOM, DEgg, DOM and HQE DOM as well as angular response curves, both of which were parameterized using the public GEANT4 tool OMSim https://github.com/icecube/OMSim. For more information on these files and how the geometry of the IceCube Upgrade sensors was chosen see Chapter 6 in my MSc thesis.
Also, the modified PPC code relevant to the angular sensitivity portion can be found in the newly created resource/PPC_executables/PPC_UPGRADE folder, specifically ini.cxx .

### Improved Muon Light Yield

Muon light yield is also changed from the following files in the prometheus folder (mainly in lepton_propagation). For more information see Chapter 5 in my MSc thesis and the following files:



- prometheus/lepton_propagation/loss.py : Added track  length to Loss class
- prometheus/photon_propagation/ppc_photon_propagator.py : Added 0m track length parameter to Loss creation to account for new Loss class type
- prometheus/lepton_propagation/new_proposal_lepton_propagator.py : Changed new_proposal_losses() to account for amu- muon loss type. Added debug logging option to verify new loss method works (need to uncomment code block at the top of file). Also added a warning and adjusted Ecut and vcut to reflect ppc parameterisations
- prometheus/utils/translators.py : Added an amu- loss type
- prometheus/utils/write_to_f2k.py : Added track length as an input to TR f2k lines

And implementation of new log-normal method is found in the newly created resource/PPC_executables/PPC_UPGRADE folder, specifically muon_secondary_params_fine.txt for the log-normal params and f2k.cxx for the implementation. Note, the log-normal implementation empirically seemed to be a bit slower, but I was not able to time-benchmark it. Also, relevant to state these parameters were found using a different index of refraction than perhaps ideal for IceCube Upgrade (check bullet point 1 of page 86 on thesis for more info). 

For specific conda env see: genie_examples/environment.yml
GENIE version: 3.04.02
There is also a .sh file in genie_examples called genie_external_root_upgrade.sh which shows you how to run a complete simulation with supplying a gtrac.root file from GENIE.

TLDR: If I were to point someone to a specific folder to look through in order to run simulations with IceCube Upgrade and/or with GENIE it would be genie_examples folder and the py/ipynb examples given there.

Also it should be noted that all of these changes were written to the version of Prometheus June 2025. So it may not be directly mergeable with the current version of Prometheus.

## *End of Preface*

Welcome to Prometheus!

Authors:

1. Jeffrey Lazar
2. Stephan Meighen-Berger
3. Christian Haack
4. David Kim
5. Santiago Giner
6. Carlos Argüelles Delgado


## Table of contents

1. [Introduction](#introduction)

2. [Dependencies](#dependencies)

3. [Installation](#installation)

    3.1. [Installation from source](#source)

    3.2. [Using Containers](#containers)

4. [Citation](#citation)

5. [Documentation](#documentation)


## Introduction <a name="introduction"></a>

`Prometheus` is a `Python`-based package for simulating neutrino telescopes. Please see [2304.14526](http://arxiv.org/abs/2304.14526) for a detailed description of the package.

## Dependencies <a name="dependencies"></a>

`Prometheus` depends a few external packages, one can either install these manually or download images with the dependencies prebuilt.
We provide details for providing either option here.

## Installation <a name="installation"></a>

To install all of `Prometheus`'s dependencies one has two options: either by downloading and compiling them from `PyPI`or source, or by downloading Docker or Singularity files.

### Installation from source <a name="source"></a>

First, we will discuss the packages which must be compiled from source.

First, we must install `LeptonInjector` which will be used to select neutrino interaction quantities.
The source code for this as well as installation instructions can be found [here](https://github.com/icecube/LeptonInjector).

Next, one may optionally install `PROPOSAL`, which is used to propagate the charged leptons that result from neutrino interactions.
We say this is optional since it can also be installed using the setup script later.
We only mention compiling it from source here because there have occasionally been issues with using `pip` for some operating systems.

Next, if one wishes to simulate ice-based detectors, it is necessary to compile the photon propagation code.
The source code for this can be found in `/resources/PPC_executables/`.
We provide two directories here, one containing code suitable for running on a CPU and one containing CUDA code that can run on a GPU.
Details on compiling them can be found in those directories.
These use a modified version of the official PPC code found [here](https://github.com/icecube/ppc).

Lastly, if one wishes to do event weighting, one must compile `LeptonWeighter`.
The source code and instructions on how to compile it can be found [here](https://github.com/icecube/LeptonWeighter).

After this, one can install all the Python dependencies by running the setup script found in the base directory of the repo, _i.e._ by running `python setup.py install` from the base directory.
There have been some issues with installing `PROPOSAL` via pip.
If this is encoutered, please see instructions for installing from source above.

### Using Containers <a name="containers"></a>

While you can install `Prometheus` manually using the raw code from here, one can also download docker or singularity image with all dependencies prebuilt from: [repo](https://drive.google.com/drive/folders/1-PbSiZQr0n85g9PrhbHMeURDOA02QUSY?usp=sharing).

Then load the docker image. In the container you will find prometheus under /home/myuserprometheus . Note you may need to run bash + source /opt/.bashrc before using prometheus.

We also offer a singularity image should you need it in the same [repo](https://drive.google.com/drive/folders/1-PbSiZQr0n85g9PrhbHMeURDOA02QUSY?usp=sharing), which may be useful for running simulations on a cluster.
This are currently in beta and require some setup:

1. Download the latest .sif file (currently v1.0.2)
2. run ``` singularity shell name_of_file.sif ```
3. enter the /opt folder ```cd /opt ```
4. source the bash file ``` source .bashrc ```
5. clone prometheus from the repository to a folder of your choice. The setup should now be done

Please note, that some systems still use older kernels not compatible with newer boost versions. In such a case use the containers with the "old" keyword.

For GPU usage the repository offers a GPU docker image, which you will need to use to build an image yourself. Then you will also have to compile ppc (using e.g. make gpu in the PPC_CUDA folder) youself. Note that you may change the arch version in the makefile of PPC to do this, depending on your hardware.


## Citation <a name="citation"></a>

Please cite this [software](https://github.com/Harvard-Neutrinoprometheus) using
```
@article{Lazar:2023rol,
    author = {Lazar, Jeffrey and Meighen-Berger, Stephan and Haack, Christian and Kim, David and Giner, Santiago and Arg\"uelles, Carlos A.},
    title = "{Prometheus: An Open-Source Neutrino Telescope Simulation}",
    eprint = "2304.14526",
    archivePrefix = "arXiv",
    primaryClass = "hep-ex",
    month = "4",
    year = "2023"
}
```

And please consider citing the packages that `Prometheus` uses internally, _i.e._ `LeptonInjector`, `PROPOSAL`, `PPC`, and `LeptonWeighter` with the following citations.

```
@article{IceCube:2020tcq,
    author = "Abbasi, R. and others",
    collaboration = "IceCube",
    title = "{LeptonInjector and LeptonWeighter: A neutrino event generator and weighter for neutrino observatories}",
    eprint = "2012.10449",
    archivePrefix = "arXiv",
    primaryClass = "physics.comp-ph",
    doi = "10.1016/j.cpc.2021.108018",
    journal = "Comput. Phys. Commun.",
    volume = "266",
    pages = "108018",
    year = "2021"
}

@article{koehne2013proposal,
  title     = {PROPOSAL: A tool for propagation of charged leptons},
  author    = {Koehne, Jan-Hendrik and
               Frantzen, Katharina and
               Schmitz, Martin and
               Fuchs, Tomasz and
               Rhode, Wolfgang and
               Chirkin, Dmitry and
               Tjus, J Becker},
  journal   = {Computer Physics Communications},
  volume    = {184},
  number    = {9},
  pages     = {2070--2090},
  year      = {2013},
  doi       = {10.1016/j.cpc.2013.04.001}
}

@misc{chirkin2022kpl,
  author = {D. Chirkin},
  title = {ppc},
  year = {2022},
  publisher = {GitHub},
  journal = {GitHub repository},
  howpublished = {\url{https://github.com/icecube/ppc}},
  commit = {30ea4ada13fbcf996c58a3eb3f0b1358be716fc8}
}
```

## Documentation <a name="documentation"></a>

API documentation can be found in `/documentation`

<!-- ## Installation <a name="installation"></a> -->
