...
#     })
    ...
    print(primary_set, prometheus_set)

    # Convert arrays to serialized strings
  ...
  #  RESOURCE_DIR = f"{'/'.join(prometheus.__path__[0].split('/')[:-1])}/resources/"
   ...
   # config["injection"]["GENIE"]["paths"]["primary file"] = h5_file_path
    ...
    config["run"]["nevents"] = len(prometheus_set)
    ...
    # injection_config = config["injection"]["GENIE"]
    # injection_config["inject"] = True
   # config["injection"]["GENIE"]["inject"] = True
    config["injection"]["GENIE"]["paths"]["injection file"] = "genie_events_for_prometheus.parquet"
