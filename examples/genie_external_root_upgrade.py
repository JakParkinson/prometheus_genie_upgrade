...
def main():
   ...
   # output_dir = f"/groups/icecube/jackp/prometheus_genie_cleaned/harvard-prometheus/examples/output"
    primrary_file_path = f'{OUTPUT_DIR}/genie_events_primary_new_test_simset_{simset}_nevents_{num_events}_seed_{seed_num}.parquet'
    prometheus_file_path = f'{OUTPUT_DIR}/genie_events_prometheus_new_test_{simset}_nevents_{num_events}_seed_{seed_num}.parquet'
    ...
    config["run"]["outfile"] = f"{OUTPUT_DIR}/{num_events}_events_simset_{simset}_seed_{seed_num}_{timestamp}.parquet"
    ...
    config["injection"]["GENIE"]["inject"] = True ## we aren't injecting using prometheus, we are using an injection file
